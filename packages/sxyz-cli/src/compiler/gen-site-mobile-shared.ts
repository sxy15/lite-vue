...
import { SRC_DIR, getSxyzConfig } from '../common/constant.js';
import {
  removeExt,
  normalizePath,
  decamelize,
...
function genConfig(demos: DemoItem[]) {
  ...
  const { nav, locales } = sxyzConfig.site;
  if (locales) {
    ...
    sxyzConfig.site.nav = demoFilter(nav);
  }

  return `export const config = ${JSON.stringify(sxyzConfig, null, 2)}`;
}
