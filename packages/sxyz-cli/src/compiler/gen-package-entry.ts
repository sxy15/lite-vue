...
import { SRC_DIR, getPackageJson, getSxyzConfig } from '../common/constant.js';
import {
  getComponents,
  normalizePath,
  pascalize,
...
export function genPackageEntry({
  ...
  const namedExport = sxyzConfig.build?.namedExport || false;
  const skipInstall = (sxyzConfig.build?.skipInstall || []).map(pascalize);
